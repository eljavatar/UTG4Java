...
foreach (@entries) {
    ...
    push(@list, {name => $_, pid => $pid, vid=>$vid, vid_num=>$vid_num, src=>$src, tid=>$tid});
}
# Ordenamos la lista
...
suite: foreach (@list) {
    ...
    # Nos quedamos solo con los que no terminan en scaffolding.java
    #@list_to_coverage = grep { !/scaffolding\.java$/ } @all_test_java_files;
    
    my $file_instrument_classes = "${PATH_INSTRUMENTS}/${pid}_${vid}_instruments_${tid}.txt";
    ...
    print "Resultado has_syntax_error: $has_syntax_error\n";
    print "Resultado test_method_name: $test_method_name\n";
    print "Resultado test_canonical_name: $test_canonical_name\n\n";
    ...
    print "CORRECT SYNTAX for test: $test_canonical_name\n\n";
    ...
    $res->{compilable} = "Yes";
    print "COMPILABLE for test: $test_canonical_name\n\n";
    ...
    $res->{executable} = "Yes";
    print "EXECUTABLE for test: $test_canonical_name\n\n";
    ...
    if (scalar(@{$list_failings->{classes}}) != 0 || scalar(@{$list_failings->{methods}}) != 0) {
        ...
        if (scalar(@{$list_failings->{classes}}) != 0) {
            ...
            print "FAILING TEST (by class) for test: $test_canonical_name\n";
        }
        ...
        if (scalar(@{$list_failings->{methods}}) != 0) {
            ...
            print "FAILING TEST (by method) for test: $test_canonical_name\n";
    ...
    print "PASSING ASSERTS for test: $test_canonical_name\n\n";
    push(@list_results, $res);
    ...
    if ($cov_results->{lines_covered} > 0) {
        ...
        print "CORRECT TEST (LINE COVERED): $test_canonical_name\n";
    }
